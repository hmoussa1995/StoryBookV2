...
Isi Design System is a reusable component library that helps Intuitive developers build UIs faster. The goal is to make building durable UIs more productive and satisfying.

## Install

Isi components are written in React, and its stories are written in [Component Story Format](https://medium.com/storybookjs/component-story-format-66f4c32366df). It requires Storybook version 5.2-beta and up.

Add SDS to your project.

`npm install @hmoussa1995/react-button-lib-github@5.0.66 --registry=https://npm.pkg.github.com/ --save-dev`
...
```
  const example = () => (
    <div>
      <IsiButton variant="Solid-High-Contrast" label="Click"/>
    </div>
  )
```
